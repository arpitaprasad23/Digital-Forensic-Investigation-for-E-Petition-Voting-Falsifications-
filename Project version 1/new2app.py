...
def record_failed_attempt(ip):
    """Increment failure count; auto-block after MAX_ATTEMPTS."""
...
def register():
    ...
    if request.method == 'POST':
        ...
        voter_number     = request.form.get('voter')
        ...
        session['temp_user'] = {
            "full_name": full_name, "email": email,
            "phone_number": phone_number, "aadhar_number": encrypted_aadhar,
            "password": pw_hash, "voter_number": voter_number,
            "date_of_birth": date_of_birth, "role": "user",
...
def confirm_address_change():
    ...
    log_action(voter_id, data['full_name'], "ADDRESS_CHANGE",
               f"Moved: {data['old_city']} → {data['new_city']}")

    cursor.execute("""
        INSERT INTO voter_accounts
            (full_name, email, phone_number, aadhar_number,
             password, voter_number, date_of_birth, role, city, state)
    ...
    """, (data['full_name'], data['email'], data['phone_number'],
          ...
          data['date_of_birth'], data['new_city'], data['new_state']))
    conn.commit()
    new_id = cursor.lastrowid
    cursor.close(); conn.close()

    log_action(new_id, data['full_name'], "REGISTERED",
               f"Re-registered after address change. New city: {data['new_city']}")
    save_snapshot()
...
def verify_otp():
    if request.form.get('otp') == session.get('otp'):
        ...
        log_action(new_id, user['full_name'], "REGISTERED",
                   f"City: {user.get('city', '')} | IP: {get_client_ip()}")
        save_snapshot()
...
def login():
    ...
    if request.method == 'POST':
        ...
        remaining = MAX_ATTEMPTS - failed_attempts.get(ip, 0)
        log_action(None, f"Aadhar:{aadhar_input[:4]}****", "LOGIN_FAILED",
                   f"Invalid credentials | IP: {ip} | Attempts left before block: {remaining}")
...
def submit_vote():
    ...
    cursor.execute("SELECT vote_id FROM ballot_records WHERE voter_id = %s", (user_id,))
    if cursor.fetchone():
        cursor.close(); conn.close()
        return jsonify({"status": "error", "message": "You have already voted!"}), 400

    cursor.execute("SELECT city FROM voter_accounts WHERE voter_id = %s", (user_id,))
    user_loc = cursor.fetchone()
    city = user_loc['city'] if user_loc else ''

    voted_at = datetime.now()
    cursor.execute(
        "INSERT INTO ballot_records (voter_id, party, party_full, voted_at, city) VALUES (%s,%s,%s,%s,%s)",
        (user_id, data.get('party'), data.get('party_full'), voted_at, city)
    )
    conn.commit()
    ...
    cursor.execute("""
        INSERT INTO vote_receipts (vote_id, voter_id, receipt_number, issued_at, expires_at, voter_ip)
        VALUES (%s, %s, %s, %s, %s, %s)
    """, (vote_id, user_id, receipt_number, voted_at, expires_at, ip))
    conn.commit(); cursor.close(); conn.close()

    log_action(user_id, session['user_name'], "VOTED",
               f"Party: {data.get('party')} | City: {city} | Receipt: {receipt_number} | IP: {ip}")
    save_snapshot()
...
def receipt(receipt_number):
    ...
    if datetime.now() > receipt_data['expires_at']:
        flash("This receipt has expired (valid for 24 hours only).", "danger")
        return redirect(url_for('dashboard'))
...
PLACES = {
    "Maharashtra": {
        "Ahmednagar": ["Ahmednagar", "Shrirampur", "Rahuri", "Kopargaon", "Sangamner", "Nevasa", "Parner", "Pathardi", "Shevgaon", "Karjat"],
        "Akola": ["Akola", "Akot", "Balapur", "Barshitakli", "Murtajapur", "Patur", "Telhara", "Washim"],
        "Amravati": ["Amravati", "Achalpur", "Anjangaon", "Daryapur", "Dhamangaon", "Morshi", "Nandgaon", "Tiosa", "Warud"],
        "Aurangabad": ["Aurangabad", "Gangapur", "Kannad", "Khuldabad", "Paithan", "Phulambri", "Sillod", "Soegaon", "Vaijapur"],
        "Beed": ["Beed", "Ambajogai", "Ashti", "Dharur", "Georai", "Kaij", "Manjlegaon", "Parli", "Patoda", "Shirur"],
        ...
        "Chandrapur": ["Chandrapur", "Ballarpur", "Bhadravati", "Brahmapuri", "Chimur", "Gondpipri", "Jivati", "Mul", "Nagbhid", "Pombhurna", "Rajura", "Sawali", "Sindewahi", "Warora"],
        "Dhule": ["Dhule", "Sakri", "Shirpur", "Sindkheda"],
        "Gadchiroli": ["Gadchiroli", "Aheri", "Armori", "Bhamragad", "Chamorshi", "Dhanora", "Etapalli", "Korchi", "Kurkheda", "Mulchera", "Sironcha"],
        ...
        "Jalgaon": ["Jalgaon", "Amalner", "Bhusawal", "Bodwad", "Chalisgaon", "Chopda", "Dharangaon", "Erandol", "Jamner", "Muktainagar", "Pachora", "Parola", "Raver", "Yawal"],
        "Jalna": ["Jalna", "Ambad", "Badnapur", "Bhokardan", "Ghansawangi", "Jafrabad", "Mantha", "Partur"],
        "Kolhapur": ["Kolhapur", "Ajra", "Bhudargad", "Chandgad", "Gadhinglaj", "Hatkanangle", "Kagal", "Karveer", "Panhala", "Radhanagari", "Shahuwadi", "Shirol"],
        "Latur": ["Latur", "Ahmadpur", "Ausa", "Chakur", "Deoni", "Jalkot", "Nilanga", "Renapur", "Shirur Anantpal", "Udgir"],
        "Mumbai City": ["Churchgate", "Fort", "Colaba", "Marine Lines", "Malabar Hill", "Walkeshwar", "Girgaon", "Grant Road", "Bhendi Bazaar", "Null Bazaar", "Dongri", "Mandvi", "Mazgaon", "Byculla", "Parel", "Dadar", "Sewri", "Wadala", "Sion", "Kurla", "Chunabhatti", "Chembur", "Govandi", "Mankhurd"],
        "Mumbai Suburban": ["Andheri", "Jogeshwari", "Vile Parle", "Santacruz", "Khar", "Bandra", "Mahim", "Worli", "Lower Parel", "Goregaon", "Malad", "Kandivali", "Borivali", "Dahisar", "Mulund", "Bhandup", "Nahur", "Vikhroli", "Ghatkopar", "Powai", "Kanjurmarg", "Sakinaka", "Marol", "Chakala"],
        "Nagpur": ["Nagpur", "Bhiwapur", "Hingna", "Kamptee", "Katol", "Kuhi", "Mauda", "Narkhed", "Parseoni", "Ramtek", "Savner", "Umred"],
        ...
        "Nandurbar": ["Nandurbar", "Akkalkuwa", "Akrani", "Nawapur", "Shahada", "Talode"],
        ...
        "Pune": ["Pune", "Baramati", "Bhor", "Daund", "Haveli", "Indapur", "Junnar", "Khed", "Maval", "Mulshi", "Purandar", "Shirur", "Velhe", "Pimpri-Chinchwad", "Hadapsar", "Kondhwa", "Kothrud", "Aundh", "Baner", "Wakad", "Hinjewadi", "Wagholi", "Undri"],
        ...
        "Solapur": ["Solapur", "Akkalkot", "Barshi", "Karmala", "Madha", "Malshiras", "Mangalvedhe", "Mohol", "North Solapur", "Pandharpur", "Sangola", "South Solapur"],
        "Thane": ["Thane", "Ambarnath", "Bhiwandi", "Kalyan", "Murbad", "Shahapur", "Ulhasnagar", "Dombivli", "Badlapur", "Titwala", "Mira-Bhayandar", "Navi Mumbai"],
        ...
        "Yavatmal": ["Yavatmal", "Arni", "Babulgaon", "Darwha", "Digras", "Ghatanji", "Kalamb", "Kelapur", "Mahagaon", "Maregaon", "Ner", "Pusad", "Ralegaon", "Umarkhed", "Wani", "Zari Jamni"],
    }
}
...
def api_places():
    ...
    if not state:
        return jsonify({"states": sorted(PLACES.keys())})
    if state and not district:
        districts = sorted(PLACES.get(state, {}).keys())
        return jsonify({"districts": districts})
    if state and district:
        cities = sorted(PLACES.get(state, {}).get(district, []))
...
def admin_dashboard():
    ...
    city_labels = [r[0] for r in city_vote_rows]
    city_counts = [r[1] for r in city_vote_rows]
...
def vote_proposal():
    ...
    if yes_count >= needed:
        ...
        conn.commit()
        flash(f"Proposal approved by {yes_count}/{total_admins} admins.", "success")
...
def account_verify():
    ...
    try:
        stored_aadhar = cipher.decrypt(user['aadhar_number'].encode()).decode()
    except Exception:
        flash("Could not verify identity.", "danger")
        return redirect(url_for('account'))

    if stored_aadhar == aadhar_input and bcrypt.check_password_hash(user['password'], password):
        session['account_verified'] = True
        log_action(session['user_id'], session['user_name'], "ACCOUNT_VERIFY",
                   "Identity verified for account settings access")
        flash("Identity verified. You may now edit your details.", "success")
    else:
        flash("Incorrect Aadhar number or password.", "danger")
...
def account_save():
    ...
    if new_password:
        ...
        cursor.execute("""
            ...
            SET full_name=%s, email=%s, phone_number=%s, date_of_birth=%s,
                city=%s, state=%s, password=%s
            WHERE voter_id=%s
        """, (full_name, email, phone_number, dob_str or None,
              city, state, pw_hash, session['user_id']))
    else:
        cursor.execute("""
            UPDATE voter_accounts
            SET full_name=%s, email=%s, phone_number=%s, date_of_birth=%s,
    ...
    log_action(session['user_id'], full_name, "ACCOUNT_UPDATED",
               f"Name: {full_name} | City: {city} | IP: {get_client_ip()}")
    save_snapshot()
